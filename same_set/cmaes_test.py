...
GENERATION_ARCH = os.getenv("GENERATION_ARCH", CURRENT_ARCH)
...
def main():
    ...
    if MODE == "generate":
        ...
        for i in range(N_IMAGES):
            ...
            attack_times.append(time.time() - t0)
            adv_images.append(adv_img)

            adv_probs = predict(adv_img)
            if np.argmax(adv_probs) == TARGET_CLASS:
                success_count += 1
        ...
        folder, suffix = "CMAES", GENERATION_ARCH 
        torch.save({
            'adversarial_images': adv_tensor,
            'original_images': images,
            'labels': labels,
            'target_class': TARGET_CLASS,
            'n_pixels': N_PIXELS,
            'generation_arch': CURRENT_ARCH
        }, f"{folder}/adversarial_images_{suffix}_{N_PIXELS}_pixels.pt")

    else:
        folder, suffix = "CMAES", GENERATION_ARCH
        data = torch.load(f"{folder}/adversarial_images_{suffix}_{N_PIXELS}_pixels.pt")
        adv_tensor = data['adversarial_images']
        images = data['original_images']   
        labels = data['labels']
        adv_images = [adv_tensor[i:i+1] for i in range(N_IMAGES)]
        for i in range(N_IMAGES):
            adv_probs = predict(adv_images[i])
            if np.argmax(adv_probs) == TARGET_CLASS:
                success_count += 1
            target_confidences.append(adv_probs[TARGET_CLASS])
            if (i+1) % 10 == 0:
